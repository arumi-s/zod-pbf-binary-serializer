...
import { fromBlocks, fromSchema, type Block } from '../../src/v4';
...
describe('fromSchema', () => {
	...
	it('should throw with unsupported type', () => {
		...
		expect(() => fromSchema(schema as any)).toThrow('Unsupported schema at: d');
	});
	...
	it('should work with z.array of ints', () => {
		...
		const data = [123, -456, 789, Number.MAX_SAFE_INTEGER, Math.floor(Number.MIN_SAFE_INTEGER / 2)];
	...
	it('should work with z.discriminatedUnion using string discriminator', () => {
		...
		expectTypeOf(decodedA).toEqualTypeOf<z.output<typeof schema>>();
		expect(decodedB).toEqual(dataB);
		expectTypeOf(decodedB).toEqualTypeOf<z.output<typeof schema>>();
	});
